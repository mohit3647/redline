...
`.scratch/` is committed, not gitignored: it is the tracker, so it has to survive a clone. This repo
is public, so anything written there is public.
...
- **Resolve**: append the answer under an `## Answer` heading, set `Status: resolved`, then append a
  context pointer to the map's Decisions-so-far in `map.md`.
